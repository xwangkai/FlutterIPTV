...
import '../../../core/utils/catchup_url_builder.dart';
...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  Future<void> _checkAndLaunchPlayer() async {
    ...
    if (PlatformDetector.isTV && PlatformDetector.isAndroid) {
      ...
      if (nativeAvailable && mounted) {
        ...
        List<bool> hasCatchupList;
        List<int> catchupDaysList;
        int currentIndex = 0;
        ...
        if (isDlnaMode) {
          ...
          hasCatchupList = [false]; // DLNA 不支持回放
          catchupDaysList = [0];
          currentIndex = 0;
        } else {
          // 正常模式：使用频道列表
          ...
          hasCatchupList = channels.map((c) => c.hasCatchup).toList();
          catchupDaysList = channels.map((c) => c.catchupDays ?? 0).toList();
        }
        ...
        final launched = await NativePlayerChannel.launchPlayer(
          ...
          hasCatchup: hasCatchupList, // 传递每个频道是否支持回放
          catchupDays: catchupDaysList, // 传递每个频道的回放天数
  ...
  void _seekCatchupTo(Duration offset) {
    ...
    final newUrl = buildCatchupUrl(seekChannel, adjustedProgram);
  ...
  void _playCatchup(EpgProgram program) {
    ...
    final catchupUrl = buildCatchupUrl(sourceChannel, program);
    if (catchupUrl == null) {
      ScaffoldMessenger.of(context).showSnackBar(
